 ...
 */
public class LoggedAutoChooser extends LoggedNetworkInput implements Sendable {
    private final String key;

    static final String NONE_NAME = "Nothing";
    private static final Alert selectedNonexistentAuto = ChoreoAlert.alert("Selected an auto that isn't an option",
        kError);
    ...
    public LoggedAutoChooser(String key) {
        ...
        periodic();
        // "This function should not be called by the user"... try me.
    ...
    private String select(String selectStr, boolean force) {
        if(Logger.hasReplaySource()) {
            // In replay, don't select values.
            nameAtGeneration = selected;
            return selected;
        }

        selected = selectStr;
        if(selected.equals(nameAtGeneration) && allianceAtGeneration.equals(DriverStation.getAlliance())) {
     ...
     *
     * <p>
     * This is done to load AutoRoutines when and only when they are selected, in order to save memory and file loading
     * time for unused AutoRoutines.
     *
     * <p>
     * The generators are only run when the DriverStation is disabled and the alliance is known.
     *
     * <p>
     * One way to keep this clean is to make an `Autos` class that all of your subsystems/resources are
     * <a href="https://en.wikipedia.org/wiki/Dependency_injection">dependency injected</a> into. Then create methods
     * inside that class that take an {@link AutoFactory} and return an {@link AutoRoutine}.
     *
     * <h3>Example:</h3>
     *
     * <pre>
     ...
     *
     * <p>
     * This is done to load autonomous commands when and only when they are selected, in order to save memory and file
     * loading time for unused autonomous commands.
     *
     * <p>
     * The generators are only run when the DriverStation is disabled and the alliance is known.
     *
    ...
    /**
     * Gets a Command that schedules the selected auto routine. This Command shares the lifetime of the scheduled
     * Command. This Command can directly be bound to a trigger, like so:
     *
     * <pre>
     * <code>
     *     AutoChooser chooser = ...;
     ...
     *
     * <p>
     * If you plan on using this {@link Command} in a {@code Trigger} it is recommended to use
